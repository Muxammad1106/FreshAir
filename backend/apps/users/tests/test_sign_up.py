...
class SignUpTest(BaseTestCase):
    ...
    def test_confirm_fail(self):
        response = self.client.post(reverse('users:confirm.tsx'), {'confirmation_code': 'fake-uuid-code'})
        self.assertEqual(404, response.status_code, response.data)

    def test_confirm_success(self):
        user = self._signup()
        user = User.objects.get(pk=user['id'])
        response = self.client.post(reverse('users:confirm.tsx'), {'confirmation_code': user.confirmation_code})
